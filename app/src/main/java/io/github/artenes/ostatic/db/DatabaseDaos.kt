...
interface AlbumDao {
    ...
    @Query("select position, id, name, files, size, added, time from top_40 inner join albums on id = album_id order by position ASC limit :limit")
    suspend fun getTop40(limit: Int = 40): List<TopAlbumView>

    @Query("select position, id, name, files, size, added, time from top_100_all_time inner join albums on id = album_id order by position ASC limit :limit")
